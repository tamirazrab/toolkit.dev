...
export const appendBlocksTool = createBaseTool({
  ...
  inputSchema: z.object({
    block_id: z.string().describe("The ID of the parent block or page to append blocks to"),
    content: z.string().describe("Markdown content to convert and append as blocks"),
    block_type: z.enum(['paragraph', 'heading_1', 'heading_2', 'heading_3', 'bulleted_list_item', 'numbered_list_item', 'to_do', 'code']).optional().describe("Type of block to create (defaults to paragraph)"),
  }),
  outputSchema: z.object({
    results: z.array(z.custom<BlockObjectResponse>()),
  }),
